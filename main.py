...
L2_WEIGHT = 0.05  # L2 regularization term weight in the final loss
SCALE_L2 = 1e-4  # A scalar multiplier for l2_regularizer
STD_DEV = 1e-2  # Random normal initializer standard deviation
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    regularizer = tf.contrib.layers.l2_regularizer(SCALE_L2)
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 4, 2,
                                        ...
                                        kernel_initializer=initializer,
                                        kernel_regularizer=regularizer)
    layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, 1,
                              padding='same',
                              kernel_initializer=initializer,
                              kernel_regularizer=regularizer)
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 4, 2,
                                        ...
                                        kernel_initializer=initializer,
                                        kernel_regularizer=regularizer)
    layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, 1,
                              padding='same',
                              kernel_initializer=initializer,
                              kernel_regularizer=regularizer)
        ...
        for image, label in get_batches_fn(batch_size):
            _,loss = sess.run([train_op, cross_entropy_loss], 
                feed_dict={input_image: image,
                           correct_label: label,
                           keep_prob: KEEP_PROB})
        print("EPOCH {} / {}. Loss = {:.3f} Time: {:.2f} s.".format(epoch+1,
            epochs, loss, time.time()-start_time))
